...
function ItemDetail({product}) {
    const {priceWithDiscount, installmentWithDiscount, installment, addProductCart} = useCartContext()
    const onAdd = (count) => {
                            ...
                            <div className="col-md-4">
                                <div className="card-body">
                                    ...
                                    <div className="text-center">
                                        {product.discount > 0 ?
                                            <>
                                                <h4 className="text-decoration-line-through d-inline-flex">${product.price}</h4>
                                                <h4 className="d-inline-flex"><b>${priceWithDiscount(product)}</b></h4>
                                            </>
                                        :
                                            <h4><b>${product.price}</b></h4>}
                                    </div>
                                    <div className="text-center mb-4">
                                        <p className="card-text"><BsCreditCard /><b> 6 cuotas sin interés </b>de ${product.discount > 0 ? installmentWithDiscount(product) : installment(product)}</p>
